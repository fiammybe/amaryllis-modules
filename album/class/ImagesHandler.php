 ...
 * @version		$Id: ImagesHandler.php 684 2012-07-07 15:02:21Z st.flohrer $
 * @package		album
 *
 */
	...
	*/
	...
	protected function beforeSave(&$obj) {
		...
		if($albumConfig['img_use_copyright'] == 1 && $obj->isNew()) {
			...
			$image = new IndexImage($img, $srcpath);
	...
	public function afterSave(&$obj) {
		...
		return TRUE;
		$img = $obj->getVar('img_url', 'e');
		if($img != "0" && $img != "" && icms_get_module_status("index")) {
		    require_once ICMS_MODULES_PATH . "/index/class/IndexImage.php";
			...
			if(!file_exists($cached_img)) {
				$srcpath = ALBUM_UPLOAD_ROOT . $this->_itemname . "/";
				$image = new IndexImage($img, $srcpath);
				$image->resizeImage($imagesConfig['image_display_width'], $imagesConfig['image_display_height'], $obj->_images_images, "100");
			}
			$cached_thumb = $obj->_images_thumbs . $images_img;
			$cached_url = $obj->_images_thumbs_url . $images_img;
			if(!is_file($cached_thumb)) {
				$srcpath = ALBUM_UPLOAD_ROOT . $this->_itemname . "/";
				$thumb = new IndexImage($images_img, $srcpath);
