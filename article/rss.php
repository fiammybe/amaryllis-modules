...
//if($articleConfig['use_rss'] == 1) {
	...
	$article_feed->category = $articleModule->getVar("name");
	
	$article_handler = icms_getModuleHandler("article", ARTICLE_DIRNAME, "article");
	$postsArray = $article_handler->getArticles(TRUE, TRUE, $clean_post_uid, FALSE, FALSE, FALSE, FALSE, FALSE,  0, $indexConfig['rss_limit'], "article_published_date", "DESC");
	
	foreach($postsArray as $postArray) {
		$article_feed->feeds[] = array (
		  'title' => $postArray['title'],
		  'link' => str_replace('&', '&amp;', $postArray['seoURL']),
		  ...
		  'guid' => str_replace('&', '&amp;', $postArray['seoURL']),
