...
include ICMS_ROOT_PATH."/modules/icmspoll/include/constants.php";
//include_once ICMS_ROOT_PATH."/class/xoopsformloader.php";
//include_once ICMS_ROOT_PATH."/class/xoopslists.php";
//include_once ICMS_ROOT_PATH."/class/xoopsblock.php";
//include_once ICMS_ROOT_PATH."/modules/icmspoll/class/icmspoll.php";
//include_once ICMS_ROOT_PATH."/modules/icmspoll/class/icmspolloption.php";
//include_once ICMS_ROOT_PATH."/modules/icmspoll/class/icmspolllog.php";
//include_once ICMS_ROOT_PATH."/modules/icmspoll/class/icmspollrenderer.php";
...
if ( $op == "list" ) {
	...
	$polls_arr =& IcmsPoll::getAll(array(), true, "weight ASC, end_time DESC", $limit+1, $start);
	icms_cp_header();
...
if ( $op == "add" ) {
	$poll_form = new icms_form_Theme(_AM_CREATNEWPOLL, "poll_form", "index.php", "post", true);
	$question_text = new icms_form_elements_Text(_AM_POLLQUESTION, "question", 50, 255);
	$poll_form->addElement($question_text);
	$desc_tarea = new icms_form_elements_Textarea(_AM_POLLDESC, "description");
	...
	$expire_text = new icms_form_elements_Text(_AM_EXPIRATION."<br /><small>"._AM_FORMAT."<br />".sprintf(_AM_CURRENTTIME, $currenttime)."</small>", "end_time", 30, 19, $endtime);
	$poll_form->addElement($expire_text);
	$disp_yn = new icms_form_elements_Radioyn(_AM_DISPLAYBLOCK, "display", 1);
	$poll_form->addElement($disp_yn);
	$weight_text = new icms_form_elements_Text(_AM_DISPLAYORDER, "weight", 6, 5, 0);
	$poll_form->addElement($weight_text);
	$multi_yn = new icms_form_elements_Radioyn(_AM_ALLOWMULTI, "multiple", 0);
	$poll_form->addElement($multi_yn);
	$anonymous_yn = new icms_form_elements_Radioyn(_AM_ALLOWANONYMOUS, "anonymous", 1); 
	$poll_form->addElement($anonymous_yn);
	$notify_yn = new icms_form_elements_Radioyn(_AM_NOTIFY, "notify", 1);
	$poll_form->addElement($notify_yn);
	$option_tray = new icms_form_elements_Tray(_AM_POLLOPTIONS, "");
	$barcolor_array = XoopsLists::getImgListAsArray(ICMS_ROOT_PATH."/modules/icmspoll/images/colorbars/");
	for($i = 0; $i < 10; $i++){
		...
		$color_select->setExtra("onchange='showImgSelected(\"option_color_image[".$i."]\", \"option_color[".$i."]\", \"modules/icmspoll/images/colorbars\", \"\", \"".ICMS_URL."\")'");
		$color_label = new icms_form_elements_Label("", "<img src='".ICMS_URL."/modules/icmspoll/images/colorbars/".$current_bar."' name='option_color_image[".$i."]' id='option_color_image[".$i."]' width='30' align='bottom' height='15' alt='' /><br />");
...
if ( $op == "edit" ) {
	$poll = new IcmsPoll($_GET['poll_id']);
	$poll_form = new icms_form_Theme(_AM_EDITPOLL, "poll_form", "index.php", "post", true);
	$author_label = new icms_form_elements_Label(_AM_AUTHOR, "<a href='".ICMS_URL."/userinfo.php?uid=".$poll->getVar("user_id")."'>".XoopsUser::getUnameFromId($poll->getVar("user_id"))."</a>");
	$poll_form->addElement($author_label);
	$question_text = new icms_form_elements_Text(_AM_POLLQUESTION, "question", 50, 255, $poll->getVar("question", "E"));
	$poll_form->addElement($question_text);
	$desc_tarea = new icms_form_elements_Textarea(_AM_POLLDESC, "description", $poll->getVar("description", "E"));
	$poll_form->addElement($desc_tarea);
	$date = formatTimestamp($poll->getVar("end_time"), "Y-m-d H:i:s");
	if ( !$poll->hasExpired() ) {
		$expire_text = new icms_form_elements_Text(_AM_EXPIRATION."<br /><small>"._AM_FORMAT."<br />".sprintf(_AM_CURRENTTIME, formatTimestamp(time(), "Y-m-d H:i:s"))."</small>", "end_time", 20, 19, $date);
		$poll_form->addElement($expire_text);
	} else {
		$restart_label = new icms_form_elements_Label(_AM_EXPIRATION, sprintf(_AM_EXPIREDAT, $date)."<br /><a href='index.php?op=restart&amp;poll_id=".$poll->getVar("poll_id")."'>"._AM_RESTART."</a>");
		$poll_form->addElement($restart_label);
	}
	$disp_yn = new icms_form_elements_Radioyn(_AM_DISPLAYBLOCK, "display", $poll->getVar("display"));
	$poll_form->addElement($disp_yn);
	$weight_text = new icms_form_elements_Text(_AM_DISPLAYORDER, "weight", 6, 5, $poll->getVar("weight"));
	$poll_form->addElement($weight_text);
	$multi_yn = new icms_form_elements_Radioyn(_AM_ALLOWMULTI, "multiple", $poll->getVar("multiple"));
	$poll_form->addElement($multi_yn);
		$anonymous_yn = new icms_form_elements_Radioyn(_AM_ALLOWANONYMOUS, "anonymous", $poll->getVar("anonymous")); 
	$poll_form->addElement($anonymous_yn);
	$options_arr =& IcmsPollOption::getAllByPollId($poll->getVar("poll_id"));
	$notify_value = 1;
	if ( $poll->getVar("mail_status") != 0 ) {
		$notify_value = 0;
	}
	$notify_yn = new icms_form_elements_Radioyn(_AM_NOTIFY, "notify", $notify_value);
	$poll_form->addElement($notify_yn);
	$option_tray = new icms_form_elements_Tray(_AM_POLLOPTIONS, "");
	$barcolor_array =& XoopsLists::getImgListAsArray(ICMS_ROOT_PATH."/modules/icmspoll/images/colorbars/");
	$i = 0;
	foreach($options_arr as $option){
		...
		$color_select->addOptionArray($barcolor_array);
		$color_select->setExtra("onchange='showImgSelected(\"option_color_image[".$i."]\", \"option_color[".$i."]\", \"modules/icmspoll/images/colorbars\", \"\", \"".ICMS_URL."\")'");
		$color_label = new icms_form_elements_Label("", "<img src='".ICMS_URL."/modules/icmspoll/images/colorbars/".$option->getVar("option_color", "E")."' name='option_color_image[".$i."]' id='option_color_image[".$i."]' width='30' align='bottom' height='15' alt='' /><br />");
		$option_tray->addElement($color_select);
		$option_tray->addElement($color_label);
		unset($color_select, $color_label, $option_id_hidden, $option_text);
		$i++;
	}
	$more_label = new icms_form_elements_Label("", "<br /><a href='index.php?op=addmore&amp;poll_id=".$poll->getVar("poll_id")."'>"._AM_ADDMORE."</a>");
...
if ( $op == "update" ) {
	...
	foreach ( $option_id as $opid ) {
		$option = new IcmsPollOption($opid);
		...
		if ( $option_text[$i] != "" ) {
			...
			if ( $option->delete() != false ) {
				IcmsPollLog::deleteByOptionId($option->getVar("option_id"));
			}
		}
		$i++;
	}
...
if ( $op == "addmore" ) {
	$poll = new IcmsPoll($_GET['poll_id']);
	$poll_form = new icms_form_Theme(_AM_ADDMORE, "poll_form", "index.php", 'post', true);
	$question_label = new icms_form_elements_Label(_AM_POLLQUESTION, $poll->getVar("question"));
...
if ( $op == "delete_ok" ) {
	...
	if ( $poll->delete() != false ) {
		IcmsPollOption::deleteByPollId($poll->getVar("poll_id"));
		IcmsPollLog::deleteByPollId($poll->getVar("poll_id"));
		include_once ICMS_ROOT_PATH.'/class/template.php';
		xoops_template_clear_module_cache($xoopsModule->getVar('mid'));
...
if ( $op == "restart" ) {
	$poll = new IcmsPoll($_GET['poll_id']);
	$poll_form = new icms_form_Theme(_AM_RESTARTPOLL, "poll_form", "index.php", 'post', true);
	$expire_text = new icms_form_elements_Text(_AM_EXPIRATION."<br /><small>"._AM_FORMAT."<br />".sprintf(_AM_CURRENTTIME, formatTimestamp(time(), "Y-m-d H:i:s"))."</small>", "end_time", 20, 19, formatTimestamp(time()+604800, "Y-m-d H:i:s"));
	$poll_form->addElement($expire_text);
	$notify_yn = new icms_form_elements_Radioyn(_AM_NOTIFY, "notify", 1);
	$poll_form->addElement($notify_yn);
	$reset_yn = new icms_form_elements_Radioyn(_AM_RESET, "reset", 0);
	$poll_form->addElement($reset_yn);
	$op_hidden = new icms_form_elements_Hidden("op", "restart_ok");
	$poll_form->addElement($op_hidden);
	$poll_id_hidden = new icms_form_elements_Hidden("poll_id", $poll->getVar("poll_id"));
	$poll_form->addElement($poll_id_hidden);
	$submit_button = new icms_form_elements_Button("", "poll_submit", _AM_RESTART, "submit");
	$poll_form->addElement($submit_button);
	icms_cp_header();
	echo "<h4>"._AM_POLLCONF."</h4>";
	$poll_form->display();
...
if ( $op == "restart_ok" ) {
	...
	if ( $reset == 1 ) {
		// reset all logs
		IcmsPollLog::deleteByPollId($poll->getVar("poll_id"));
		IcmsPollOption::resetCountByPollId($poll->getVar("poll_id"));
	}
	if (!$poll->store()) {
		echo $poll->getHtmlErrors();
		exit();
...
if ( $op == "log" ) {
	...
	// show logs
	$logs_arr =& IcmsPollLog::getAllByPollId($poll_id, $orderby." ".$orderdir);
	$logs_count = count($logs_arr);
	$arrow_up = ICMS_URL."/modules/icmspoll/images/up.gif";
	$arrow_down = ICMS_URL."/modules/icmspoll/images/down.gif";
...
//		echo "<td nowrap>&nbsp;</td>";
		...
		for ( $i = 0; $i < $max; $i++ ) {
			$option = new IcmsPollOption($logs_arr[$i]->getVar("option_id"));
			echo "<tr class='bg1'>";
			...
			if ($logs_arr[$i]->getVar("user_id") != 0) {
				...
				echo "<td align='center'><a href=".ICMS_URL."/userinfo.php?uid=".$user->getVar("uid").">".$uname."</a></td>";
			} else {
